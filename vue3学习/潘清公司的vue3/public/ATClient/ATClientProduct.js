...
function show_ext_status(frmDetails, strExt, strStatus) {
    ...
    var myCallInfo = ATGetUidInfo_byExt(strExt);
...
function Popup_Callin(EventCode, EventData) {
    ...
    {
        var myCallInfo = ATGetCallInfo(login_uid, EventData, "", "");
...
function AT_Command(strCmd) {
    ...
    var myCallInfo = ATGetCallInfo(login_uid, "", "", "");
    var strCallid = myCallInfo.m_callid;
    switch (strCmd) {
        // 摘机
        case "Pickup":
            ATAnswer(login_uid, strCallid);
            break;
        // 挂机
        case "Hangup":
            ATHangup(login_uid, strCallid);
            break;
        // 代接
        case "Pickup_dj":
            ATPickCall(login_uid, $("#txtTel").val(), strCallid);
            break;
        case "PlaceCall": //呼叫电话
        ...
        // 恢复呼叫
        case "RetriveCall":
            ATRetriveCall(login_uid, strCallid);
            break;
        // 单步转接
        case "TransCall": //单步转接
        ...
        case "ConfOver": //协商会议完成
            ATConfOver(login_uid, strCallid);
            break;
