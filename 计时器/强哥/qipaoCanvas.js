const c = document.getElementById('time_canvas')
...
//使小球移动(向上做曲线运动)
function moveBall() {
    cContext.clearRect(0, 0, c.offsetWidth, window.innerHeight);
    for (let i = 0; i < nums; i++) {
        (function (i) {
            if (ballK[i] == null) {
                ballK[i] = 0;
            }
            reX = ballK[i] * ballS[i] + ballY[i];
            reY = Math.sin(ballK[i]) * ballF[i] + ballX[i];
            if (reX + ballR[i] <= 0) {
                ballY[i] = window.innerHeight + 20;
                ballK[i] = 0;
                reX = ballK[i] * ballS[i] + ballY[i];
            }
            cContext.beginPath();
            cContext.fillStyle = ballC[i];
            cContext.globalAlpha = 0.5;
            cContext.arc(reY, reX, ballR[i], 0, Math.PI * 2, true);
            cContext.fill();
