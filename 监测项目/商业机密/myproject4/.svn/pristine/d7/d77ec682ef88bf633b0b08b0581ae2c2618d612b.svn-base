<template>
  <div class="rosterWarning_levelConfiguration">
     <el-form :model="searchform"  ref="searchForm" label-width="140px" >
          <el-row>
           <el-col :span="8">
              <el-form-item  label="紧急程度"  prop="codeName">
            <el-input clearable v-model="searchform.codeName" maxlength="30" placeholder="紧急程度"></el-input>
          </el-form-item>
           </el-col>
      ...
      <el-table style="width: 100%;margin-top:20px" :data="tableData">
          ...
          <el-table-column prop="codeName" label="紧急程度" min-width="110"></el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="dialogFormVisible">
        <el-form :rules="rules" :model="form" ref="form">
          ...
          <el-form-item label="紧急程度" :label-width="formLabelWidth" prop="codeName">
            <el-input maxlength="30" clearable v-model="form.codeName" placeholder="紧急程度"></el-input>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    MYgetsourceBusinessArr() {
      ...
      MYgetsourceBusinessArr(this.params, '1').then(res => {
        this.loadingA = false
        if (res.code === 200) {
    ...
    exportExcel() {
      if (this.tableData.length > 0) {
        location.href = `/monitor/clueManage/codeDispose/outFile/1?token=${this.token}`
      } else {
        this.$message({
          message: '列表为空无法导出',
          type: 'warning',
          ...
          if (this.dialogName === '编辑') {
            modification(this.id, this.form).then(res => {
              if (res.code === 200) {
                ...
                this.MYgetsourceBusinessArr(this.paramsObj, '1')
              }
            })
          } else {
