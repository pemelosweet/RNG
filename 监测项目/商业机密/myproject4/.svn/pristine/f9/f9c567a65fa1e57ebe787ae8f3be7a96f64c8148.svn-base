  ...
  element-loading-background="rgba(0, 0, 0, 0.3)">
    <el-card class="box-card">
      ...
      >
        ...
        <router-link
          style="float:right"
          :to="{name:'fenResultExport_add'}"
        >
          <el-button type="text">发起协查</el-button>
        </router-link>
      </div>
      <div class="text item">
            ...
            <el-row class="toggle" :gutter="20">
              ...
              <el-col :span="8">
                ...
                >
                  <el-select
                    clearable
                    v-model="formInline.assistState"
                  >
                  <el-option
                      label="编辑中"
                      value="0"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div>
                {{scope.row.assistState ==='0'?'编辑中':scope.row.assistState ==='1'?'查询中':scope.row.assistState ==='2'?'已完成':'已反馈'}}
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="300" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="text" :disabled="scope.row.assistState ==='1' || (new Date().getTime() - new Date(scope.row.submitTime).getTime())/86400000 >= 30"  @click="deleteInfo(scope.row)">删除结果</el-button>
              <el-button type="text" :disabled="scope.row.assistState ==='0' || scope.row.assistState ==='1'"  @click="addInfo(scope.row)">填写反馈情况</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
import { deleteInfoById } from '@/api/sys-monitoringAnalysis/placeResultExport/index'
import { getList, getListFor } from '@/api/sys-monitoringAnalysis/placeResultExport/index'
...
export default {
  ...
  computed: {
    ...
    searchParams() {
      ...
      if (this.formInline.rangeDate !== null && this.formInline.rangeDate !== '' && this.formInline.rangeDate.length > 0) {
        obj.receDateStart = this.formInline.rangeDate[0]
        obj.receDateEnd = this.formInline.rangeDate[1]
      }
      if (this.formInline.feedDate !== null && this.formInline.feedDate !== '' && this.formInline.feedDate.length > 0) {
        obj.feedDateStart = this.formInline.feedDate[0]
        obj.feedDateEnd = this.formInline.feedDate[1]
      }
      delete obj.feedDate
  ...
  methods: {
    ...
    downloadInfo(scope) {
      this.$confirm('部门负责人是否同意下载', '提示', {
        ...
        .then(() => {
          window.location.href = '/monitor/areaAssist/exportResult2Zip/' + scope.id + '?token=' + this.token
        })
        .catch(res => {})
    },
    // 下载列表
    downloadList() {
      if (this.multipleSelection.length === 0) {
        ...
        window.location.href = '/monitor/areaAssist/export2Excel?token=' + this.token + '&dataIds=' + this.multipleSelection.join(',')
      }
    },
    // 全选
    ...
    deleteInfo(scope) {
      this.$confirm('确定删除选中的记录?', '提示', {
        ...
        deleteInfoById(scope.id).then(res => {
          if (res.code === 200) {
            this.$message({
              type: 'success',
              message: '删除成功',
    ...
    cleanUp() {
      this.formInline.assistName = null
      this.formInline.assistState = null
      this.formInline.rangeDate = []
      this.formInline.feedDate = []
