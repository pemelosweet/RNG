<template>
  <div class="forewarningRule">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>预警规则管理</span>
        <router-link to="forewarningRule/add" style="float: right">
          <el-button type="text" icon="el-icon-plus"> 新增规则</el-button>
        </router-link>
      </div>
      <div class="content">
        <el-form :model="form" ref="searchForm" label-width="114px" :rules="searchRules" class="demo-form-inline">
          <el-row :gutter="20">
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="预警程度" prop="wornLevel">
                <el-select v-model="form.wornLevel" clearable placeholder="请选择">
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="预警周期" prop="wornCycle">
                <el-select v-model="form.wornCycle" clearable placeholder="请选择">
            ...
            <el-col :span="12">
              <el-form-item label="监测周期" prop='monitorCycle'>
                <el-date-picker value-format="yyyy-MM-dd" v-model="form.monitorCycle" type="daterange" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions">
                </el-date-picker>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="5">
              <organization-tree v-on:showParentComp="getTemArr" type="rule" :lableWidth="114" ref="tree"></organization-tree>
            </el-col>
            <el-col :span="19">
              <el-tag v-for="(itme,index) in userArr" :key="index" closable @close="handleClose(itme,index)" style="marginLeft:15px;marginBottom:15px;marginTop:5px">
                {{itme.name}}
              </el-tag>             
            </el-col>
          </el-row> 
        ...
        <el-table 
          ...
          <el-table-column prop="creUser" label="创建人" min-width="120" show-overflow-tooltip></el-table-column>
          <el-table-column prop="option" label="操作" width="140" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="text" @click="deleteSingleRule(scope.row.ruleId)">删除</el-button>
...
export default {
  ...
  computed: {
    searchParams() {
      ...
      obj.mCycleStart = this.form.monitorCycle ? this.form.monitorCycle[0] : ''
      obj.mCycleEnd = this.form.monitorCycle ? this.form.monitorCycle[1] : ''
      // 拿到属性组件的参数
      if (this.$refs.tree) {
        this.temArr = this.$refs.tree.listWarnRangeDO.map(item => {
          return item.id
        })
        obj.rangeIds = this.temArr.join()
      } else {
        obj.rangeIds = ''
      }
      return obj
  ...
  methods: {
    ...
    cleanUp() {
      ...
      this.userArr = []
      this.$refs.tree.source.data = []
      this.$refs.tree.target.data = []
      this.$refs.tree.listWarnRangeDO = []
    },
    ...
    viewSingle(id) {
      ruleDetail(id).then(res => {
        if (res.code === 200) {
          ...
          // for (let i = 0; i < this.singleRuleData.length; i++) {
          //   if (this.singleRuleData[i].param_value || this.singleRuleData[i].param_value === '') {
          //     console.log('--------------------' + i)
          //   }
          //   if (this.singleRuleData[i].param_value || this.singleRuleData[i].param_value === '') {
          //     console.log('--------------------' + i)
    ...
    handleClose(item, index) {
      console.log(this.$refs.tree.listWarnRangeDO)
