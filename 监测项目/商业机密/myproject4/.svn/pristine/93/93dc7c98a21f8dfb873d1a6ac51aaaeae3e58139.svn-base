      ...
      >
          ...
          <el-table :data="form.tableData" max-height="500px" style="margin-left: -100px;" border>
            ...
            <el-table-column label="状态" min-width="80"  prop="taskStatus">
              <template slot-scope="scope">
                <div>
                  {{scope.row.taskStatus ==='1'?'分析中':scope.row.taskStatus ==='2'?'分析完成':scope.row.taskStatus ==='3'?'申请上报分析':'已分发'}}
                </div>
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  type="text"
                  :disabled="scope.row.taskStatus==='1'||scope.row.taskStatus==='2'||scope.row.taskStatus==='3' || isEnd"
                  @click="startFen(scope.row)"
                >开始分析</el-button>
                <el-button
                  type="text"
                  :disabled="scope.row.taskStatus!=='1' || isEnd"
                  @click="stopFen(scope.row)"
                >结束分析</el-button>
                <el-button
                  :disabled="scope.row.taskStatus==='4' || scope.row.taskStatus ==='2' || isEnd"
                  type="text"
                  @click="toFen(scope.row)"
                >申请上报分析</el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-dialog title="任务信息" :visible.sync="editVisible" width="60%">
            <el-form ref="formMain" :model="formMain">
              <el-form-item label="报文名称：" label-width="200px" prop="taskName" :rules="[{required: true, validator: delDataValidInput, trigger: 'blur'}]">
                  <el-input v-model="formMain.taskName" maxlength="50"  placeholder="请输入报文名称,最多输入50字符"></el-input>
              </el-form-item>
              <el-form-item label="是否需上报分析：" :rules="[{required: true, trigger: 'blur'}]" label-width="200px">
                  <el-select
                    clearable
...
export default {
  ...
  methods: {
    ...
    // 下载模板
    download() {
      window.location.href = '/analyse-task/download?token=' + this.token
    },
    // 下载附件
    downFile(file) {
      ...
      location.href = '/file-service/upload/download/' + file.id + '?moduleName=' + encodeURI('分析任务')
    },
    // 上传文件
    upSuccess(res, file) {
      if (res.code === 200) {
