  ...
  <div class="calendar">
    <div class="calendar-tools">
      ...
      <div class="calendar-info" @click.stop="changeYear">
        <!-- {{monthString}} -->
        <div class="month">
          <div class="month-inner" :style="{'top':-(this.month*30)+'px'}">
    ...
    <slot name="operation" :backToday="setToday">
      <el-button @click="setToday" type="primary" plain style="margin-left:20px">返回今日</el-button>
    </slot>
    <table cellpadding="5">
      <thead>
        <tr>
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}" :key="k1">
          <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)" :key="k2">
            <span :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">
              {{child.day}}
              <sub v-if="child.eventName!=undefined&&child.eventName=='班'" style="color:#409EFF">{{child.eventName}}</sub>
              <sub v-else style="color:#67C23A">{{child.eventName}}</sub>
            </span>
...
export default {
  ...
  methods: {
    ...
    // 渲染日期
    render(y, m) {
      const firstDayOfMonth = new Date(y, m, 1).getDay() // 当月第一天 星期
      const lastDateOfMonth = new Date(y, m + 1, 0).getDate() // 当月最后一天
      const lastDayOfLastMonth = new Date(y, m, 0).getDate() // 上个月的最后一天
      this.year = y
        ...
        if (this.range) {
          ...
          if (this.end.length > 0) {
            const endTime = Number(
              new Date(parseInt(this.end[0]), parseInt(this.end[1]) - 1, parseInt(this.end[2]))
            )
    ...
    setToday() {
      const now = new Date()
      this.ParentYear = now.getFullYear() // 小霞加
      this.month = now.getMonth()
      this.day = now.getDate()
      const value = this.ParentYear
      this.render(this.year, value)
