  ...
  <div class="rosterManage-viewRoster">
    <el-card>
      <div slot="header" class="clearfix">
        <span>创建人：{{form.creUser?form.creUser:''}}</span>
        <span style="float:right;">创建时间：{{form.infoSaveDate}}</span>
      </div>
      <div style="margin:20px 0">
        <span>名单类型：</span>
        <span>{{rosterType}}</span>
      </div>
      <el-form :model="form" ref="ruleForm">
        ...
        <el-row :gutter="20">
          <el-col :span="24" v-if="form.charosterPreliminaryJudgmentDOList.length>0">
            <el-form-item label="初步判断：">
              <span style="margin-right:18px" v-for="(item,index) in form.charosterPreliminaryJudgmentDOList" :key="index">{{item.preliminaryJudgme}}</span>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
        ...
        <el-form-item label="授权机构：">
          <el-tag style="margin-left:10px" v-for="(item,index) in form.powerDOS " :key="index" type="info">{{item.powName}}</el-tag>
        </el-form-item>
        <el-form-item label="授权结果：">
          <span v-if="form.powerDOS.length>0">{{ form.powerDOS[0].powResult |  authoriation }}</span>
        </el-form-item>
...
<script>
import { getSingle } from '@/api/sys-monitoringAnalysis/roster-warning/authorization.js'
export default {
  data() {
    return {
      rosterType: '主体类型名单',
  ...
  created() {
    ...
    const powResult = this.$route.query.powResult
    getSingle(id, powResult).then(res => {
      if (res.code === 200) {
        if (res.data.powerDOS.length > 0) {
          if (res.data.powerDOS[0].mechanismName === null) {
            res.data.powerDOS = []
          }
        }
        ...
        // if (this.fLetterTypeArr.length > 0) {
        //   this.fLetterTypeArr = this.form.fLetterType.split(',')
        // }
