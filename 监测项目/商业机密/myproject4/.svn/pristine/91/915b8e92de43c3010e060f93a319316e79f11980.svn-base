  ...
  computed: {
    ...
    searchParams() {
      ...
      if (this.dialogForm.preliminaryJudgment.indexOf('1402') !== -1) {
        ...
        this.dialogForm.preliminaryJudgment[n] = '1402-' + this.supplementTwo
      }
      const preliminaryJudgment = this.dialogForm.preliminaryJudgment
  ...
  watch: {
    ...
    formContent: {
      handler(newVal, oldVal) {
        ...
        for (var i = 0; i < newVal.listWarnDOS.length; i++) {
          const current = newVal.listWarnDOS[i]
          if (current.wType) {
            if (current.wType === 'warnMain') {
              this.showTableItem = 'file'
            } else if (current.wType === 'center') {
        ...
        this.dialogForm.preliminaryJudgment = newVal.preliminaryJudgme
        this.fetchData()
