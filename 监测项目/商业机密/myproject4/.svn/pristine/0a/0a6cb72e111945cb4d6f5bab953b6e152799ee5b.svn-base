<template>
  <div class="rosterWarning_levelConfiguration">
    <el-card>
      <div slot="header" class="clearfix">
        ...
        <div  style="float:right">
           <router-link :to="{name:'cueManage_levelConfiguration_batchImport',query:{id:this.activeName}}">
      ...
      <el-tab-pane label="来文单位" name="first">
       <el-form :model="searchform"  ref="searchForm" label-width="140px" >
          <el-row :gutter="20">
           <el-col :span="8">
              <el-form-item  label="来文单位"  prop="codeName">
            <el-input clearable v-model="searchform.codeName" maxlength="30" placeholder="来文单位"></el-input>
          </el-form-item>
           </el-col>
            <el-col :span="8">
      ...
      <el-table style="width: 100%;margin-top:20px" :data="tableData">
          ...
          <el-table-column prop="codeName" label="来文单位" min-width="110"></el-table-column>
    ...
    <el-tab-pane label="紧急程度" name="second">
      <direction ref="direction"/>
    </el-tab-pane>
    <el-tab-pane label="发送单位" name="three">
      <subject ref="subject"/>
    </el-tab-pane>
    <el-tab-pane label="触发来源" name="trigger">
      <trigger ref="trigger"/>
    </el-tab-pane>
  </el-tabs>
   <el-dialog :title="dialogName" :visible.sync="dialogFormVisible">
        <el-form :rules="rules" :model="form" ref="form">
          <el-form-item label="序号" v-if="activeName==='first'" :label-width="formLabelWidth" prop="typeNumber">
          ...
          <el-form-item label="序号" v-if="activeName==='trigger'" :label-width="formLabelWidth" prop="typeNumber">
            <el-input maxlength="6" clearable v-model="form.typeNumber" placeholder="请填写"></el-input>
          </el-form-item>
          <el-form-item v-if="activeName==='first'" label="来文单位" :label-width="formLabelWidth" prop="codeName">
            <el-input clearable v-model="form.codeName" maxlength="30" placeholder="来文单位"></el-input>
          </el-form-item>
           <el-form-item v-if="activeName==='second'" label="紧急程度" :label-width="formLabelWidth" prop="codeName">
            <el-input clearable v-model="form.codeName"  maxlength="30" placeholder="紧急程度"></el-input>
          </el-form-item>
          <el-form-item v-if="activeName==='three'" label="发送单位" :label-width="formLabelWidth" prop="codeName">
            <el-input clearable v-model="form.codeName"  maxlength="30" placeholder="发送单位"></el-input>
          </el-form-item>
          <el-form-item v-if="activeName==='trigger'" label="触发来源" :label-width="formLabelWidth" prop="codeName">
            <el-input clearable v-model="form.codeName"  maxlength="30" placeholder="触发来源"></el-input>
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    exportExcel() {
      if (this.tableData.length > 0) {
        ...
        location.href = `/monitor/clueManage/codeDispose/outFile/${id}?token=${this.token}`
      } else {
        this.$message({
          message: '列表为空无法导出',
          type: 'warning',
    ...
    handleClick(tab, event) {
      if (tab.index === '0') {
        ...
        this.$refs.trigger.MYgetsourceBusinessArr()
      }
    },
    query() {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.activeName === 'first') {
            ...
            obj.typeName = '发送单位字典'
          } else if (this.activeName === 'trigger') {
            obj.typeName = '触发来源字典'
          }
            ...
            add(obj).then(res => {
              if (res.code === 200) {
                ...
                if (this.activeName === 'first') {
                  ...
                  this.$refs.direction.MYgetsourceBusinessArr(this.paramsObj, '1')
                } else if (this.activeName === 'three') {
                  this.$refs.subject.MYgetsourceBusinessArr(this.paramsObj, '2')
                } else if (this.activeName === 'trigger') {
                  this.$refs.trigger.MYgetsourceBusinessArr(this.paramsObj, '3')
                }
              }
            })
          }
