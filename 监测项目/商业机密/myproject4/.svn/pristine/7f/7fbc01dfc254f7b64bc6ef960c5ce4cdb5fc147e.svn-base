...
import { FollowUpdate } from '@/api/sys-monitoringAnalysis/judgedClues/add.js'
...
export default {
  ...
  methods: {
    init() {
      this.form = this.tableForm
      // 涉罪类型
      var Arr = this.tableForm.crimeType.split(',')
      // 涉罪类型
      const psclueIdArr = []
      Arr.forEach(el => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.tabindex === 0) {
            submitObj.typeFlag = 1
          } else {
            // submitObj.typeFlag = 0
          }
