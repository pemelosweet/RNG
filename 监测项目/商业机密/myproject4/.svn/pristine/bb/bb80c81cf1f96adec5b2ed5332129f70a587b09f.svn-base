<template>
  <el-card class="box-card rosterManage-batchImport">
    ...
    <div class="content">
      <el-upload class="upload-container" accept=".csv,.xls,.xlsx" :show-file-list="false" drag :file-list="fileList" :before-upload="beforeUpload"  :on-progress="handleProgress" :on-preview="onPreview" :on-success="onSuccess" :on-error="onError" :action="uploadURL" multiple :auto-upload="autoUpload">
      ...
      <el-form :inline="true"  v-if="historyHide" :model="form" ref="ruleForm">
...
export default {
  ...
  computed: {
    ...
    uploadURL() {
      return `/monitor/listwarn/center/import?username=${this.user_name}&token=${this.token}`
    }
  },
  mounted() {
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      return new Promise(function(resolve, reject) {
        verifyFileName({ impName: encodeURI(file.name) })
          .then(res => {
            if (res.code === 200) {
              if (res.data === 'false') {
                _this
    ...
    downloadXlsFn() {
      location.href = '/monitor/listwarn/center/downloadXls?token=' + this.token
    },
    // 查询历史文件
