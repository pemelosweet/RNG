...
<div style='width:100%;height:100%;background-color:#fff;position: relative'>
  ...
  >
    <el-card>
      <div slot="header"><span>支付机构大额统计分析</span>
        ...
        >
          <el-col :span="24">
            <el-col :span="12">
              ...
              >
                <el-select clearable style="width:100%;" v-model="form.statisticalType" placeholder="请选择统计类型(必填)" @change="handleChange">
                  <el-option label="按报告机构统计" value="RICD"></el-option>
                  <el-option label="按行业统计" value="INDUSTRY"></el-option>
                  <el-option label="按大额特征统计" value="STCR"></el-option>
                  <el-option label="按上报网点所在地统计" value="TRCD"></el-option>
                  <el-option label="按报告金额段统计" value="CRATPART"></el-option>
                  <el-option label="按账户类型统计" value="CATP"></el-option>
                  <el-option label="按业务种类统计" value="TSTP"></el-option>
                  <!-- <el-option label="按渉罪类型统计" value="TOSC"></el-option> -->
                  <el-option label="按客户职业或行业统计" value="CTTP"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
      ...
      <el-table
        ...
        <el-table-column prop="tosc" min-width="80" v-if="otosc" label="涉罪类型" sortable="custom" key='3'>
         ...
         <el-table-column prop="stcr" min-width="80" v-if="ostcr" label="大额交易特征" sortable="custom" key='4'>
            <template slot-scope="scope">
              <el-tooltip  effect="dark" placement="top-start">
        ...
        <el-table-column prop="largeNumberOfReports" min-width="80" v-if="olargeAmountOfReport" sortable="custom" label="大额报告份数" key='13'></el-table-column>
        <el-table-column prop="transactionPenNumber" min-width="100" v-if="otransactionPenNumber" sortable="custom" label="大额交易笔数" key='15'></el-table-column>
        <el-table-column prop="largeAmountOfReportStr" min-width="100" v-if="olargeAmountOfReport" label="大额交易总金额(万元)" sortable="custom" key='14'></el-table-column>
        <el-table-column prop="accountNumber" min-width="110" v-if="oaccountNumber" label="大额交易涉及账户数 " sortable="custom" key='16'></el-table-column>
        <el-table-column prop="numberOfSubjects" min-width="120" v-if="onumberOfSubjects" label="大额交易涉及主体数" sortable="custom" key='17'></el-table-column>               
        <el-table-column prop="transactionCharacteristicCode" v-if="oTransactionCharacteristicCode" min-width="80" label="大额交易特征" key='18'></el-table-column>          
...
} from '@/api/sys-monitoringAnalysis/statisticForm/payingOrg.js'
import {
  getIndustryFrist,
  getIndustrySecond,
  endTimes,
    ...
    return {
      ...
      chartSettings: {
        ...
        labelMap: {
          ...
          CATP: '账户类型',
          FINNNAME: '资金流向',
          largeNumberOfReports: '大额交易报告份数',
          largeAmountOfReport: '大额交易报告金额',
          accountNumber: '大额交易报告涉及账户数',
          transactionPenNumber: '大额交易报告笔数',
          numberOfSubjects: '大额交易报告涉及交易主题数'
        }
      },
      ...
      token: getToken(),
      title: [
        '分支行所统计的数据范围为：本分支行辖区内报告机构网点所报送的大额交易报告数据。',
        '由于支付大额表结构与可疑表相同，固按照统计可疑交易的逻辑进行统计，因此均取通过二级校验的数据，即报文的Xml_type取值为新增N/重发R/补报A/补正I，再去除trade_state为待补正（trade_state=2）/因补正失效（按期)(trade_state=3）/因补正失效（延期)(trade_state=4）/删除前待补正（延期)(trade_state=7）的数量。统计时间段均为落地日期，包含首尾日。',
        '1、大额交易报告份数：通过二级校验的报告数COUNT（xml_ID)',
        '2、大额交易报告总金额（万元）：统计满足条件下支付大额表中，所有交易的总金额sum(CRAT_RMB)',
        '3、大额交易报告交易笔数：统计条件中，通过可疑报文表字段：UDSI区分 值=KKK1 or KKK2 or KKK3 or KKK4 为大额报文的“可疑交易数量（ttnm）”',
        '4、大额交易报告涉及账户数：统计支付大额交易表中，所有交易的交易方的账户总数(去重标准：按账户号与账户类型均相同记为同一账户的规则报文内去重)',
        '5、大额交易报告涉及主体数：通过报文表UDSI区分 值=KKK1 or KKK2 or KKK3 or KKK4 为大额报文的“可疑主体数量（sctn）”'
      ],
      formRules: {
  ...
  methods: {
    ...
    getData() {
      ...
      getIndustryFrist()
        ...
        .catch(() => {})
      administrativeDivision({ type: 'ZFJGDETJFX' })
        .then(res => {
          if (res.code === 200) {
    ...
    exportStatistics() {
      ...
      if (length === 0) {
        ...
        if (ids) {
          ...
          obj.ids = ids
          location.href =
            '/monitor/payorg/statistics/export?statisticalType=' +
            obj.statisticalType +
            '&starStatisticalTime=' +
      ...
      if (this.list.length > 0) {
        this.$confirm('此操作将导出此统计条件下的所有数据, 是否继续?', '提示', {
          ...
          .then(() => {
            ...
            location.href =
              '/monitor/payorg/statistics/all?statisticalType=' +
              obj.statisticalType +
              '&starStatisticalTime=' +
              obj.statisticalTime[0] +
              '&endStatisticalTime=' +
