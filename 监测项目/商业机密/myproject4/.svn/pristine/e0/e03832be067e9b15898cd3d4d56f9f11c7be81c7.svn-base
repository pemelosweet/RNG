...
//  bushi
  ...
  StreamProvider.prototype = {
    ...
    updateFileList: function(a) {
      // var fileCertInfo = certificate.getSingature(a.target.value,'file')
      // var fileCexrtInfo = a.target.value
  ...
  StreamUploader.prototype = {
        ...
        case 'load':
          ...
          try {
            if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 308)) {
              ...
              console.log(respJson,'111')
              // if (respJson.start === undefined) {
          ...
          if (uploaded < this.get('size') - 1) {
            ...
            this.streamUpload(uploaded)  // streamUpload1
          } else {
    ...
    startUpload: function(url, postVars, fileFieldName) {
      ...
      this.fileStartPosValue = null
      this.retriedTimes = 0
      console.log(postVars, 11111234567)
      postVars.name = this.get('name')
      postVars.attachSize = this.get('size')
      console.log(typeof postVars.attachSize)
  ...
  Main.prototype = {
    ...
    upload: function(index) {
      ...
      xhr.onreadystatechange = function() {
        ...
        try {
          try {
            token = eval('(' + xhr.responseText + ')').data
            // server = eval('(' + xhr.responseText + ')').data
