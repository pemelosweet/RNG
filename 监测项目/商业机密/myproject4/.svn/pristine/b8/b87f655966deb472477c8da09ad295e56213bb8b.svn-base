...
;(function() {
  var Provider,
    nIdCount = 0,
    aOtherBrowsers = ['Maxthon', 'SE 2.X', 'QQBrowser'],
    nZero = 0,
    sOneSpace = ' ',
    sLBrace = '{',
    sRBrace = '}',
    ...
    pa =
      Array.isArray && /\{\s*\[(?:native code|function)\]\s*\}/i.test(Array.isArray)
        ? Array.isArray
        : function(a) {
    ...
    sCellFileTemplate =
      ...
      '</div>',
    sTotalContainer =
      '<div id="#totalContainerId#" class="stream-total-tips">' +
      ...
      '</div>',
    sFilesContainer =
      '<div class="stream-files-scroll" style="height: #filesQueueHeight#px;"><ul id="#filesContainerId#"></ul></div>'
  ...
  function fExtend(a, b) {
    ...
    return function() {
      var d = 'string' === typeof a ? b[a] : a,
        e = c ? [arguments[0]].concat(c) : arguments
  ...
  function fAddVars(json, url, c) {
    ...
    if (!c && url)
      ...
      for (var g = 0, c = json.length; g < c; ++g) f(json[g], g)
    else if ('undefined' != typeof json && null !== json && 'object' === typeof json)
      for (g in json) f(json[g], g)
    else _array.push(encodeURIComponent(url) + '=' + encodeURIComponent(json))
    return _array
      .join(_sep)
      .replace(/^&/, '')
      .replace(/%20/g, '+')
  ...
  function fMergeJson(base, extend) {
    var result = {}
    for (var attr in base) result[attr] = base[attr]
    for (var attr in extend) result[attr] = extend[attr]
    return result
  }
  ...
  function fRemoveClass(element, klass) {
    element &&
      (element.className = element.className
        .replace(RegExp('(^| )' + klass + '( |$)'), ' ')
        .replace(/^\s+|\s+$/g, ''))
  }

  function fContains(container, klass) {
    if (!container) return []
    if (container.querySelectorAll) return container.querySelectorAll('.' + klass)
  ...
  function fShowMessage(msg, warning) {
    ...
    o &&
      (o.innerHTML += '<br>' + (!!warning ? "<span style='color:red;'>" + msg + '</span>' : msg)) &&
      (o.scrollTop = o.scrollHeight)
  }

  function fMessage(msg, VarVals, c, d) {
    for (
      var nLIndex, nRIndex, sepIndex, argName, secondVar, s = [], _argName, length = msg.length;
      ;
      ...
      if (nLIndex + 1 >= nRIndex) break
      argName = _argName = msg.substring(nLIndex + 1, nRIndex)
      secondVar = null
      sepIndex = argName.indexOf(sOneSpace)
      ;-1 < sepIndex &&
        ((secondVar = argName.substring(sepIndex + 1)), (argName = argName.substring(0, sepIndex)))
    ...
    return msg
      .replace(ga, function(msg, VarVals, c) {
        return sLBrace + s[parseInt(c, 10)] + sRBrace
      })
      .replace(rLBrace, sLBrace)
      .replace(rRBrace, sRBrace)
  }

  function fIsObjOrFun(a, b) {
    var c = typeof a
    return (
      (a && ('object' === c || (!b && ('function' === c || 'function' === fToString(a))))) || !1
    )
  }
  ...
  function fGetStyle(el, name) {
    if (window.getComputedStyle) return window.getComputedStyle(el, null)[name]
  ...
  function fRegFuns(hostPrototype, funs, forseReg, d) {
    var e, f, g
    if (!hostPrototype || !funs) return hostPrototype || {}
    if (d)
      for (e = 0, g = d.length; e < g; ++e)
        (f = d[e]),
          Object.prototype.hasOwnProperty.call(funs, f) &&
            (forseReg || !(f in hostPrototype)) &&
            (hostPrototype[f] = funs[f])
    else {
      for (f in funs)
        Object.prototype.hasOwnProperty.call(funs, f) &&
          (forseReg || !(f in hostPrototype)) &&
          (hostPrototype[f] = funs[f])
      ;({ valueOf: 0 }.propertyIsEnumerable('valueOf') ||
        fRegFuns(
          hostPrototype,
          funs,
          forseReg,
          'hasOwnProperty,isPrototypeOf,propertyIsEnumerable,toString,toLocaleString,valueOf'.split(
            ','
          )
        ))
    }
      ...
      this.constructor.prototype,
      {
        ...
        fire: function(a) {
          ...
          if (b) {
            ...
            if (fIsObjOrFun(d[0]))
              for (var e in c) d[0][e] ? (d[0]['_' + e] = c[e]) : (d[0][e] = c[e])
            else d[0] = c
  ...
  function Parent(args) {
    ...
    if (args) {
      for (var field in args) this.set(field, args[field])
    }
  ...
  Parent.prototype = {
    ...
    set: function(key, val) {
      ...
      key &&
        ...
        ((this.config[key] = val),
        (c = key + 'Change'),
        this._evt && c in this._evt.events && this.fire(c))
    }
  }

  function SWFReference(a, b, c) {
    fRegEvents.call(this)
    var d = (this._id = fGenerateId('uploader-swf')),
      c = c || {},
      e = ((c.version || sFlashVersion) + '').split('.'),
      e = SWFReference.isFlashVersionAtLeast(
        parseInt(e[0], 10),
        parseInt(e[1], 10),
        parseInt(e[2], 10)
      ),
    ...
    if (a && (e || f) && g) {
      b += 'id="' + d + '" '
      b = Browser.ie
        ? b + ('classid="' + sIEFlashClassId + '" ')
        : b + ('type="' + sShockwaveFlash + '" data="' + g + '" ')
      ...
      for (var s in c.flashVars)
        (j = c.flashVars[s]), 'string' === typeof j && (h += '&' + s + '=' + encodeURIComponent(j))
      h && (b += '<param name="flashVars" value="' + h + '"/>')
      a.innerHTML = b + '</object>'
      this.swf = document.getElementById(d)
    } else
      this.publish('wrongflashversion', { fireOnce: !0 }),
        this.fire('wrongflashversion', { type: 'wrongflashversion' })
  }
  ...
  SWFReference.prototype = {
    ...
    _eventHandler: function(a) {
      'swfReady' === a.type
        ? (this.publish('swfReady', { fireOnce: !0 }), this.fire('swfReady', a))
  ...
  SWFProvider.prototype = {
    ...
    renderUI: function(a) {
      ...
      var b = fCreateContentEle(
        "<div id='" +
          this.swfContainerId +
          "' style='position:absolute;top:0px; left: 0px; margin: 0; padding: 0; border: 0; width:100%; height:100%'></div>"
      )
    ...
    setContainerClass: function(a, b) {
      b
        ? fAddClass(this.contentBox, this.get('containerClassNames')[a])
        : fRemoveClass(this.contentBox, this.get('containerClassNames')[a])
    },
    setFileFilters: function() {
      
      this.swfReference &&
        0 < this.get('fileFilters').length &&
        this.swfReference.callSWF('setFileFilters', [
          [
            {
              description: '\u81ea\u5b9a\u4e49\u6587\u4ef6',
              extensions: '*' + this.get('fileFilters').join(';*'),
              macType: '*' + this.get('fileFilters').join(';*')
            }
          ]
        ])
    },
    setMultipleFiles: function() {
      this.swfReference &&
        this.swfReference.callSWF('setAllowMultipleFiles', [this.get('multipleFiles')])
    },
    triggerEnabled: function() {
      this.get('enabled')
        ? (this.swfReference.callSWF('enable'),
          this.swfReference.swf.setAttribute('aria-disabled', 'false'))
        : (this.swfReference.callSWF('disable'),
          this.swfReference.swf.setAttribute('aria-disabled', 'true'))
    },
    ...
    upload: function(uploader, url, postVars) {
      ...
      var url = url || this.get('uploadURL'),
        postVars = fMergeJson(postVars, this.get('postVarsPerFile')),
        id = uploader.id,
    ...
    startUpload: function(url, postVars, fileFieldName) {
      ...
      if (this.get('uploader')) {
        var uploader = this.get('uploader'),
          fileFieldName = fileFieldName || 'FileData',
          id = this.get('id'),
  ...
  StreamProvider.prototype = {
    ...
    renderUI: function(a) {
      this.contentBox = a
      this.fileInputField = fCreateContentEle(
        "<input type='file' style='visibility:hidden;width:0px;height:0px;opacity:0;position:absolute;left:-1000px;'>"
      )
      this.get('dragAndDropArea') &&
        !this.get('dragAndDropArea').nodeType &&
        this.set('dragAndDropArea', document.getElementById(this.get('dragAndDropArea')))
      bDraggable &&
        this.get('dragAndDropArea') &&
        (fAddClass(this.get('dragAndDropArea'), 'stream-browse-drag-files-area'),
        this.get('dragAndDropArea').appendChild(fCreateContentEle(this.get('dragAndDropTips'))))
    },
    bindUI: function() {
    ...
    bindDropArea: function() {
      ...
      null !== a &&
        (fAddEventListener(a, 'drop', this.dropBinding),
    ...
    destroy: function() {
      ...
      null !== a &&
        ...
        (fRemoveEventListener(a, 'drop', this.dropBinding),
    ...
    dragEventHandler: function(evt) {
      ...
      // evt.stopPropagation ? evt.stopPropagation() : evt.cancelBubble = !0;  取消冒泡
      // if(this.config.dropFn){
      // 	this.config.dropFn(evt.type)
      // }
      //
      switch (evt.type) {
        case 'dragenter':
        ...
        case 'dragover':
          ...
          break
        case 'dragleave':
          this.config.dropFn(false)

          // if (evt.screenX === 0 && evt.screenY === 0 && !evt.fromElement && evt.offsetX <= 0) {
          // 	this.config.dropFn(false)
          // }
        ...
        case 'drop':
          ...
          if (bFolder) {
            ...
            if (items.length && items[items.length - 1]) {
              var entry =
                items[items.length - 1].webkitGetAsEntry() || items[items.length - 1].getAsEntry()
              var entries = []
    ...
    _handleFileTreeEntry: function(entry, callback) {
      ...
      var that = this,
        reader,
        path = entry.fullPath.replace(/^\//, '').replace(/(.+?)\/?$/, '$1'),
        entries = [],
        succeed = function(entries) {
    ...
    bindSelectButton: function() {
      ...
      for (var i = 0, len = this.contentBox.length; i < len; i++) {
        var current = this.contentBox[i]
        current.index = i
        fAddEventListener(this.contentBox[current.index], 'click', this.buttonBinding)
      }
    },
    setMultipleFiles: function() {
      !0 === this.get('multipleFiles') && this.fileInputField.setAttribute('multiple', 'multiple')
    },
    setFileFilters: function() {
      
      var a = this.get('fileFilters')
      0 < a.length
        ? this.fileInputField.setAttribute('accept', a.join(','))
        : this.fileInputField.setAttribute('accept', '')
    },
    triggerEnabled: function() {
      var a = this.get('dragAndDropArea')
    ...
    updateFileList: function(a) {
      ...
      var fileCertInfo = ''
    //   try {
    //       fileCertInfo = certificate.getSingature(a.target.value, 'file')
    //       if (fileCertInfo.indexOf('+') !== -1) {
    //         fileCertInfo = fileCertInfo.replace(/\+/g, '#')
    //       }
    //   } catch (error) {
    //     fileCertInfo = ''
    //   }
      //  var fileCertInfo = a.target.value
      for (var a = a.target.files, b = [], c = 0; c < a.length; c++) {
        // certificate
        if (a[c].name == '.') continue
        a[c].fileCertInfo = fileCertInfo
    ...
    upload: function(uploader, url, postVars) {
      var url = url || this.get('uploadURL'),
        postVars = fMergeJson(postVars, this.get('postVarsPerFile')),
        d = uploader.id,
        postVars = postVars.hasOwnProperty(d) ? postVars[d] : postVars
  ...
  StreamUploader.prototype = {
    ...
    initializer: function(file) {
      ...
      var b = StreamUploader.isValidFile(file)
        ? file
        : StreamUploader.isValidFile(file.file)
          ? file.file
          : !1
    ...
    formUpload: function() {
      ...
      var fd = new FormData(),
        fileFileName = this.get('fileFieldName'),
    ...
    streamUpload: function(pos) {
      ...
      // 第六步
      var _url = this.get('uploadURL') + '&fileCertInfo' + '=' + encodeURIComponent(this.file.fileCertInfo)
      this.resetXhr()
      this.resume = false
      this.bytesStart = pos
      this.XHR = new XMLHttpRequest()
      ...
      _xhr.onreadystatechange = function () {
        ...
        try {
          try {
            jsonData = JSON.parse(_xhr.responseText)
    ...
    uploadEventHandler: function(event) {
      ...
      switch (event.type) {
        case 'load':
          ...
          break
        case 'error':
          this.retriedTimes < this.retryTimes
            ? this.retry()
            : this.fire('uploaderror', {
                originEvent: event,
                status: xhr.status,
        ...
        case 'abort':
          ...
          break
        case 'progress':
          var total = this.get('size'),
            loaded = this.bytesStart + event.loaded,
            now = new Date().getTime(),
            cost = (now - this.preTime) / 1e3,
            totalSpeeds = 0
          if (0.68 <= cost || 0 === this.bytesSpeeds.length) {
            ...
            this.bytesSpeeds.push(this.bytesSpeed)
            for (var i = 0; i < this.bytesSpeeds.length; i++) totalSpeeds += this.bytesSpeeds[i]
    ...
    startUpload: function(url, postVars, fileFieldName) {
      ...
      postVars.attachSize = this.get('size')
    ...
    sliceFile: function(f, startPos, endPos) {
      ...
      endPos = endPos || 0
      return f.slice
        ? f.slice(startPos, endPos)
        : f.webkitSlice
          ? f.webkitSlice(startPos, endPos)
          : f.mozSlice
            ? f.mozSlice(startPos, endPos)
            : f
    },
    cancelUpload: function() {
  ...
  Main.prototype = {
    ...
    initializer: function() {
      ...
      this.startPanel =
        this.config.browser || document.getElementsByClassName(this.config.browseFileId)
      /** the default UI */
      if (!this.config.customered) {
        ...
        var filesQueue = fCreateContentEle(
            sFilesContainer
              .replace('#filesContainerId#', filesContainerId)
              .replace('#filesQueueHeight#', this.config.filesQueueHeight)
          ),
          totalQueue = fCreateContentEle(
            sTotalContainer.replace('#totalContainerId#', totalContainerId)
          )
    ...
    addStreamTask: function(a) {
      var file_id = a.get('id'),
        cell_file = fCreateContentEle("<li id='" + file_id + "' class='stream-cell-file'></li>")
      ...
      if (!this.config.customered) {
        this.getNode('stream-file-name', cell_file).getElementsByTagName(
          'strong'
        )[0].innerHTML = a.get('name')
    ...
    hideBrowseBlock: function() {
      ...
      !this.browseFileBlockDisplay &&
        (this.browseFileBlockDisplay =
          this.startPanel.style.display == '' ? 'block' : this.startPanel.style.display)
      bStreaming
    ...
    showBrowseBlock: function() {
      ...
      // // 修改了 获取css样式方法  style改为 getComputedStyle
      // !this.browseFileBlockDisplay && (this.browseFileBlockDisplay = document.defaultView.getComputedStyle(this.startPanel, null).display == "" ? "block" : this.startPanel.style.display);
    ...
    bindUI: function(file_id) {
      var b = this.uploadInfo[file_id].progressNode,
        cancelBtn = this.getNode('stream-cancel', b)
      this.cancelBtnHandler = fExtend(this.cancelUploadHandler, this, {
        type: 'click',
        nodeId: file_id
      })
    ...
    completeUpload: function(info) {
      ...
      this.containerPanel &&
        this.containerPanel.parentNode &&
        ((scroll = this.containerPanel.parentNode),
      ...
      this.waiting.length == 0 &&
        (this.get('onQueueComplete')
          ? this.get('onQueueComplete')(info.msg)
          : this.onQueueComplete(info.msg))
      this.config.autoRemoveCompleted &&
        ((info = document.getElementById(info.id)),
        info.parentNode && info.parentNode.removeChild(info))
      this.uploading = !1
      this.upload()
    },
    ...
    //   fShowMessage(
    //     'File:' + file.name + ' Size:' + file.size + ' has been UPLOADED or ADDED.',
    //     true
    //   )
    //   return false
    // },
    ...
    onMaxSizeExceed: function(file) {
      fShowMessage(
        'File:' + file.name + ' size is:' + file.size + ' Exceed limited:' + file.limitSize,
        true
      )
    },
    ...
    disable: function() {
      this.fileProvider.set('enabled', !1),
        this.fileProvider.triggerEnabled(),
        fAddClass(this.startPanel.children[0], 'stream-disable-browser'),
        fAddClass(this.startPanel, 'disabled')
    },
    enable: function() {
      this.fileProvider.set('enabled', !0),
        this.fileProvider.triggerEnabled(),
        fRemoveClass(this.startPanel.children[0], 'stream-disable-browser'),
        fRemoveClass(this.startPanel, 'disabled')
    },
    stop: function() {
      if (!this.uploading) return false
      this.uploading = !1
      var files = this.uploadInfo,
        number = 0
    ...
    cancel: function() {
      ...
      var files = this.uploadInfo,
        number = 0
      /** cancel the unfinished uploading files */
      for (var fileId in files) !files[fileId].uploadComplete && ++number && this.cancelOne(fileId)
      this.get('onCancelAll') ? this.get('onCancelAll')(number) : this.onCancelAll(number)
    },
    cancelOne: function(file_id, stopping) {
      var provider = this.uploadInfo[file_id].file,
        actived = this.uploadInfo[file_id].actived
      ...
      var totalSize = this.totalFileSize - this.uploadInfo[file_id].file.config.size,
        info = {
          id: file_id,
      ...
      if (actived) {
        ...
        for (var i in this.waiting) if (this.waiting[i] === file_id) this.waiting.splice(i, 1)
      }
      ...
      if (this.totalFileSize === 0) percent = 100
      ...
      if (!this.config.customered) {
        ...
        this.getNode('stream-process-bar', this.totalContainerPanel).innerHTML =
    ...
    cancelUploadHandler: function(event, b) {
      ...
      if (this.uploadInfo[id].disabled) return !1
    ...
    unloadHandler: function(evt) {
      ...
      if (this.waiting.length > 0)
        return (evt.returnValue =
          '\u60A8\u6B63\u5728\u4E0A\u4F20\u6587\u4EF6\uFF0C\u5173\u95ED\u6B64\u9875\u9762\u5C06\u4F1A\u4E2D\u65AD\u4E0A\u4F20\uFF0C\u5EFA\u8BAE\u60A8\u7B49\u5F85\u4E0A\u4F20\u5B8C\u6210\u540E\u518D\u5173\u95ED\u6B64\u9875\u9762')
    },
    upload: function(index) {
      ...
      var file = this.uploadInfo[index].file,
        self = this
      ...
      //   size: file.get('size'),
      //   modified: file.get('dateModified') + '',
      //   fileCertInfo: file.file.fileCertInfo,
      // }
      ...
      xhr.onreadystatechange = function() {
        ...
        try {
          try {
            ...
            // message = eval('(' + xhr.responseText + ')').message
          } catch (e) {}
          ...
          if (token) {
            ...
            bStreaming && bFileSlice && !self.config.formed
    ...
    uploadProgress: function(a) {
      var id = a.target.get('id'),
        percent = Math.min(99.99, a.percentLoaded),
        totalPercent =
          ((this.totalUploadedSize + a.bytesLoaded) * 10000) / this.totalFileSize / 100,
        totalPercent = Math.min(99.99, totalPercent)
      ...
      var progressNode = this.uploadInfo[id].progressNode,
        ...
        loaded = this.formatBytes(bytesLoaded),
        total = this.formatBytes(a.bytesTotal),
        _remainTime = this.formatTime(a.remainTime),
        a = Math.min(99.99, a.percentLoaded)
      100 > a && (a = parseFloat(a).toFixed(2))
      this.getNode('stream-process-bar', progressNode).innerHTML =
      ...
      if (_remainTime) this.getNode('stream-remain-time', cellInfosNode).innerHTML = _remainTime
      ...
      var percent = ((this.totalUploadedSize + bytesLoaded) * 10000) / this.totalFileSize / 100,
        percent = Math.min(99.99, percent)
      100 > percent && (percent = parseFloat(percent).toFixed(2))
      this.getNode('_stream-total-uploaded', this.totalContainerPanel).innerHTML = _loaded
      this.getNode('stream-percent', this.totalContainerPanel).innerHTML = percent + '%'
      this.getNode('stream-process-bar', this.totalContainerPanel).innerHTML =
    ...
    uploadComplete: function(a) {
      ...
      var id = a.target.get('id'),
        percent = Math.min(100, a.percentLoaded),
    ...
    uploadError: function(evt) {
      this.get('onUploadError')
        ? this.get('onUploadError')(evt.status, evt.statusText)
        : this.onUploadError(evt.status, evt.statusText)
    },
    fileSelect: function(a) {
      var a = a.fileList,
        ...
        c,
        files = []
      for (c = 0; c < a.length; c++) files.push(a[c].config)
      ...
      if (b == this.get('simLimit') || a.length > this.get('simLimit')) {
        this.get('onFileCountExceed')
          ? this.get('onFileCountExceed')(Math.max(a.length, b), this.get('simLimit'))
          : this.onFileCountExceed(Math.max(a.length, b), this.get('simLimit'))
        return !1
      }
      for (c = 0; c < a.length; c++) this.validateFile(a[c]) && this.addStreamTask(a[c])
    },
    validateFile: function(uploader) {
      var name = uploader.get('name'),
      ...
      //     if (key === unique) {
      //       var ignore = this.get('onRepeatedFile')
      //         ? this.get('onRepeatedFile')(info)
      //         : this.onRepeatedFile(info)
      //       if (!ignore) return !1
      //     }
      //   }
      // }
      ...
      if (this.get('maxSize') < size)
        ...
        if (!valid)
          this.get('onExtNameMismatch')
            ? this.get('onExtNameMismatch')(info)
            : this.onExtNameMismatch(info)
      }
      valid &&
        this.config.customered &&
        this.get('onAddTask') &&
        this.get('onAddTask')(info, uploader)
      return valid
    },
    formatSpeed: function(a) {
      var b = 0
      1024 <= Math.round(a / 1024)
        ? ((b = Math.round(100 * (a / 1048576)) / 100),
          (b = Math.max(0, b)),
          (b = isNaN(b) ? 0 : parseFloat(b).toFixed(2)),
          (a = b + 'MB/s'))
        : ((b = Math.round(100 * (a / 1024)) / 100),
          (b = Math.max(0, b)),
          (b = isNaN(b) ? 0 : parseFloat(b).toFixed(2)),
          (a = b + 'KB/s'))
      return a
    ...
    formatTime: function(time) {
      var total = time || 0,
        ...
        minute = Math.floor((total - 3600 * hour) / 60),
        second = Math.floor(total - 3600 * hour - 60 * minute),
        hour =
          '' + (!isNaN(hour) && 0 < hour ? (hour < 10 ? '0' + hour + ':' : hour + ':') : '00:'),
        hour =
          hour +
          (!isNaN(minute) && 0 < minute ? (minute < 10 ? '0' + minute + ':' : minute + ':') : '00:')
      return (hour +=
        !isNaN(second) && 0 < second ? (second < 10 ? '0' + second + '' : second) : '00')
    },
  ...
  var sIEFlashClassId = 'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000',
    sShockwaveFlash = 'application/x-shockwave-flash',
    sFlashVersion = '10.0.22',
    sFlashDownload =
  ...
  var Browser = (function(a) {
    ...
    if (a) {
      ...
      if ((d = a.match(/AppleWebKit\/([^\s]*)/)) && d[1]) {
        e.webkit = b(d[1])
        if ((d = a.match(/Version\/([^\s]*)/)) && d[1]) e.safari = d[1]
        if (/PhantomJS/.test(a) && (d = a.match(/PhantomJS\/([^\s]*)/)) && d[1])
          e.phantomjs = b(d[1])
        if (/Mobile\//.test(a) || /iPad|iPod|iPhone/.test(a)) {
          if (
            ...
            (d = a.match(/iPad|iPod|iPhone/)) && d[0])
          )
            e[d[0].toLowerCase()] = e.ios
        } else {
          if ((d = a.match(/NokiaN[^\/]*|webOS\/\d\.\d/))) e.mobile = d[0]
          if (/webOS/.test(a) && ((e.mobile = 'WebOS'), (d = a.match(/webOS\/([^\s]*);/)) && d[1]))
            e.webos = b(d[1])
          if (/ Android/.test(a)) {
          ...
          if (/Silk/.test(a)) {
            ...
            if (!e.android) (e.android = 2.34), (e.os = 'Android')
      ...
      if (!e.webkit)
        if (/Opera/.test(a)) {
          ...
          if (
            /Opera Mobi/.test(a) &&
            ((e.mobile = 'opera'),
            (d = a.replace('Opera Mobi', '').match(/Opera ([^\s]*)/)) && d[1])
          )
            e.opera = b(d[1])
          if ((d = a.match(/Opera Mini[^;]*/))) e.mobile = d[0]
        } else if ((d = a.match(/MSIE\s([^;]*)/)) && d[1]) e.ie = b(d[1])
        else if ((d = a.match(/Gecko\/([^\s]*)/))) {
          if (((e.gecko = 1), (d = a.match(/rv:([^\s\)]*)/)) && d[1])) e.gecko = b(d[1])
          if ((d = a.match(/(Firefox)\/([^\s]*)/)) && d[1] && d[2]) e.firefox = d[2]
        }
    }
    ...
    if (fIsArray(f)) {
      if (fIsNumber(parseInt(f[0], 10))) e.flashMajor = f[0]
      if (fIsNumber(parseInt(f[1], 10))) e.flashMinor = f[1]
      if (fIsNumber(parseInt(f[2], 10))) e.flashRev = f[2]
    }
    return e
  })()
  var bFileSlice = !1
  ...
  var bStreaming = (function() {
      ...
      if (
        ...
        (bFileSlice =
          ...
          'mozSlice' in File.prototype ||
          'webkitSlice' in File.prototype)
      )
        bHtml5 = !0
      ;(function() {
    ...
    bDraggable = bStreaming && 'draggable' in document.createElement('span'),
    bFolder = bDraggable && (window.webkitRequestFileSystem || window.requestFileSystem)
