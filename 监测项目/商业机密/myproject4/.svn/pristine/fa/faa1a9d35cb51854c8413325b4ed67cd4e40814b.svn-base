            ...
            <el-col :span="8">
              ...
              >
                <el-select
                  ...
                  clearable
                >
                 <el-option v-for="(item,idx) in  countryOptions" :label="item.text" :value="item.code" :key="idx"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
             <el-col :span="8">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                type="text"
                v-if="scope.row.subject!=='后端数据录入'&&powers"
              ...
              >发起流程</el-button>
                ...
                type="text"
                 :disabled="scope.row.oneClick"
                  v-if="scope.row.approvalStatus=== '7' && scope.row.fileType !== '4'"
                @click="handleSend(scope)"
              >发送</el-button>
                <!-- <el-button
                v-if="scope.row.fileType==='1'&& scope.row.approvalStatus=== '7'"
                type="text"
...
export default {
  data() {
    return {
      ...
      pathTableList: [
        // { flowexId: '0', flowexName: '国际发送情报流程' },
  ...
  methods: {
    ...
    handleSend(scope) {
      ...
      window.open(`${this.interAderrss}?type=monitor&mailId=${scope.row.mailId}&statsPage=drafts`, '_blank')
      // location.href = `${this.interAderrss}?type=monitor&mailId=${scope.row.mailId}&statsPage=drafts`
    },
    // 查看流程
    handleViewPress(scope) {
      const titleCode =
        scope.row.intelBusinessType = '国际发送情报流程'
      this.$router.push({
    ...
    handleGo(scope) {
      ...
      this.pathTableList = [
        { flowexId: '2', flowexName: '国际发送情报流程' }
      ]
      // switch (scope.row.intelBusinessType) {
      //   case '0':
      //     this.pathTableList = [{ flowexId: '0', flowexName: '国际发送情报流程' }]
      //     break
      //   // case '1':
      ...
      //   default:
      //     this.pathTableList = [
      //       { flowexId: '3', flowexName: '国际发送情报流程' }
      //       // { flowexId: '1', flowexName: '国际协查反馈函报中心领导流程' }
      //     ]
      //     break
      // }
    ...
    getData() {
      ...
      if (this.searchFlag) {
        const obj1 = {
          ...
          startCreateTime: this.formInline.createDate ? this.formInline.createDate[0] : '',
          endCreateTime: this.formInline.createDate ? this.formInline.createDate[1] : '',
          startSendTime: this.formInline.sendDate ? this.formInline.sendDate[0] : '',
          endSendTime: this.formInline.sendDate ? this.formInline.sendDate[1] : '',
