...
// 批量删除
export function batchDelete(id, modelTable) {
  return request({
    url: 'caml-model/modelData/batchdelete?taskId=' + id + '&tableName=' + modelTable,
    method: 'delete'
  })
}
// 启动
export function batchStart(modelTable, params, cModelTable, taskId) {
  return request({
    url: 'caml-model/createData/batchStart2?dataTemplateId=' + params + '&&modelTable=' + modelTable + '&&cModelTable=' + cModelTable + '&&taskId=' + taskId,
    method: 'post'
  })
}
...
// 执行日志
export function performLog(taskId, modelTable) {
  return request({
    url: 'caml-model/modelData/selectByLog?taskId=' + taskId + '&modelTable=' + modelTable,
    method: 'get'
  })
}

// 添加数据集
export function addDataCentratSets(params, connType) {
  return request({
    url: 'caml-model/modelData/add?connType=' + connType,
    method: 'post',
    data: params
  })
}
