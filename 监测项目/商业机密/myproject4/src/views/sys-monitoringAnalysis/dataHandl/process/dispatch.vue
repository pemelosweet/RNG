<template>
  <el-card>
    ...
    <el-table :data="list.slice((currentPage-1)*pagesize,currentPage*pagesize)">
        ...
        <el-table-column type="index" label="调度ID" width="100" fixed></el-table-column>
        ...
        <el-table-column prop="isRun" label="是否重复执行" min-width="120"></el-table-column>
        <el-table-column prop="expression" label="Cron表达式" min-width="100"></el-table-column>
        <el-table-column prop="option" fixed="right" label="操作" min-width="80">
          <template slot-scope="scope">
            <el-button type="text">删 除</el-button>  <!--v-if="scope.row.fkqk==='待反馈'"-->
          </template>
        </el-table-column>
    </el-table>
