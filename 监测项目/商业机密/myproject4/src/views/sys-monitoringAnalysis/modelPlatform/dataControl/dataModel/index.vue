<template>
  <div class="dataModel">
    ...
    <el-dialog title="新建数据模板" :visible.sync="disNewDataModel" class="personDialog" :close-on-click-modal="false" width="70%" @close="closeDialog">
      ...
      <el-form ref="newDataModelsOne" :model="newDataModels" :rules="rulesNewData" label-width="120px">
    ...
    <!--保存-->
    <el-dialog title="" :visible.sync="dialogStorageVisible" width="40%" class="personDialog" :close-on-click-modal="false">
...
export default {
  ...
  created() {
    this.bussinessTimeArr = window.transactionTimeArr
  },
  ...
  methods: {
    ...
    cancelData() {
      ...
      this.$refs.newDataModelsOne.resetFields()
    },
    newDataTemplate() {
    ...
    // 组装数据
    assemblyData() {
      this.$refs.newDataModelsOne.validate((valid) => {
        if (valid) {
    ...
    handleDtCheckAllChange(val) {
      ...
      const Data = this.bussinessTimeArr
