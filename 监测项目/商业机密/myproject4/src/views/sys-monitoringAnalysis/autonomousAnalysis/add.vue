  ...
  <div class="reportLibraryHadle" 
    ...
    <el-card>
      ...
      <el-form ref="validForm" :rules="rules" :model="form" disabled>
        ...
        <div
          class="block"
          v-if="transferHide&&((this.isCenter&&form.clewState!=='已审核')||(this.isBranch&&this.form.isvisible==='1'&&(form.reviewOpinion==='10'||form.reviewOpinion==='11'||form.reviewOpinion==='12')))"
        >
          <el-row class="blckTitle">
            <el-col :span="12">
              <span>移送情况</span>
          ...
          <div class="itemBlock">
            ...
            <!-- <div class="itemBlock">
            <div>
              <el-form-item label="主要交易发生地：" label-width="160px">
                <el-select
                :disabled="this.ysAsh"
                  @change="countryOrProvince2"
                  v-model="form.ysdistrictFlag"
                  placeholder
                  style="width:100px"
                  class="districtFlag"
                >
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="移送时间：" label-width="220px">
                  <el-date-picker
                    :disabled="this.ysAsh"
                    v-model="form.transferTime"
                    placeholder="选择时间"
        ...
        <div class="block" v-if="followHide&&((this.form.isvisible==='1'&&this.isBranch&&(form.reviewOpinion==='10'||form.reviewOpinion==='11'||form.reviewOpinion==='12'))||this.isCenter)">
          <el-row class="blckTitle">
            <el-col :span="12">
              <span>后续办理情况</span>
            </el-col>
            ...
            <el-row :gutter="20">
              <el-col :span="8">
                <el-form-item label="线索状态：" label-width="160px" prop="withState">
                  <el-select v-model="form.withState" placeholder="请选择" :disabled="this.hxAsh">
      ...
      <div class="saveBtn">
        ...
        <el-button
          ...
          v-if="form.clewState==='已审核'&&(this.isBranch&&(form.reviewOpinion==='10'||form.reviewOpinion==='11'||form.reviewOpinion==='12'))&&this.signs"
          plain
        >保 存</el-button>
        <el-button
          :loading="loads"
        ...
        <el-button
          ...
          v-if="followHideB&&this.signs&&this.form.isvisible==='1'&&(form.reviewOpinion==='10'||form.reviewOpinion==='11'||form.reviewOpinion==='12')&&this.hxlengths<2"
          plain
        >保 存</el-button>
        <el-button type="primary" v-if="followHideB&&this.signs&&(form.reviewOpinion==='10'||form.reviewOpinion==='11'||form.reviewOpinion==='12')&&this.form.isvisible==='1'" plain @click="hxadd">添加后续办理情况</el-button>
        <!-- <router-link :to="{name:'judgedClues_subsequent_subsequentAdd',query:{ id: this.id}}">
          <el-button type="primary" v-if="followHideB&&this.signs" plain>添加后续办理情况</el-button>
        </router-link>-->
        <router-link
          :to="{name: 'judgedClues_subsequent_subsequentHistory',query:{ id: this.id,person:this.person}}"
        >
          <el-button type="primary" v-if="followHideB&&(form.reviewOpinion==='10'||form.reviewOpinion==='11'||form.reviewOpinion==='12')&&this.form.isvisible==='1'" plain>查看后续办理情况</el-button>
        </router-link>
        <el-button type="primary" @click="closeT" plain>返 回</el-button>
      </div>
    </el-card>
...
export default {
  ...
  mounted() {
    this.sbypxsId = this.$route.query.sbypxsId
    if (this.sbypxsId !== '' && this.sbypxsId !== null && this.sbypxsId !== undefined) {
      this.getData()
      this.getDetail()
  ...
  methods: {
    // 下载附件
    downFile(file) {
      location.href = '/file-service/upload/download/' + file.attachId + '?moduleName=上报分析申请'
    },
    // 删除附件
    handleRemove(file, fileList) {
      if (file && file.status === 'success') {
        if (file.response) {
          this.delUrl = 'file-service/upload/delete-attach/' + file.response.data.attachId + '?moduleName=上报分析申请'
        } else if (file.attachId) {
          this.delUrl = 'file-service/upload/delete-attach/' + file.attachId + '?moduleName=上报分析申请'
        }
    ...
    hxadd() {
      if (this.hxlengths > 1) {
        this.$router.push({ name: 'judgedClues_subsequent_subsequentAdd', query: { id: this.id }})
      } else {
        this.$message({
          message: '请填写当前后续表单',
          type: 'warning',
