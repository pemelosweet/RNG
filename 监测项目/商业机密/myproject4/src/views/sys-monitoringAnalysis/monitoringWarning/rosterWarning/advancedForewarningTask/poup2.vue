  ...
  data() {
    return {
      ...
      rules: {
        ...
        suspiciousAccout: [{ validator: ValidQueryInput, trigger: 'blur' }],
        certificateNum: [{ min: 6, message: '请输入至少6位字符数', trigger: 'blur' }, { validator: ValidQueryInput, trigger: 'blur' }]
      }
    }
  },
  computed: {
    ...
    certificate() {
      switch (this.dialogForm.certificateType) {
        ...
        case '110001':
          ...
          break
        case '':
          this.rules.certificateNum = [{ validator: ValidQueryInput, trigger: 'blur' }]
          break
        default:
          this.rules.certificateNum = [{ min: 6, message: '请输入至少6位字符数', trigger: 'blur' }, { max: 128, validator: ValidQueryInput, trigger: 'change' }]
          break
      }
    }
  },
