<template>
  <div class="forewarningTask-poup1" :key="1">
    <!-- 弹框内容 -->
    <div>
      <el-form :model="dialogForm" ref="searchForm" label-width="100px">
        <el-row :gutter="20">
            <!-- <el-col :span="8">
              <el-form-item label="主体名称：" prop="subjectName" :rules="[{validator: isValidInput, trigger: 'blur'}]">
                <el-input v-model="dialogForm.subjectName"  placeholder="最长50字符数" maxlength="128" clearable></el-input>
              </el-form-item>
            </el-col>
           <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="证件号码：" prop="subjectINum">
              <el-input v-model="dialogForm.subjectINum"  placeholder="最长128字符数" maxlength="128" clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
           <el-col :span="8">
              <el-form-item label="账户号码：" prop="accNum" :rules="[{validator: isValidInput, trigger: 'blur'}]">
                <el-input v-model="dialogForm.accNum" placeholder="最长64字符数" maxlength="64" clearable></el-input>
              </el-form-item>
          </el-col> -->
          
          <el-col :span="24">
              <!-- <preliminary-judgment :lableWidth="114" ref="judgment"></preliminary-judgment> -->
                <el-form-item label="涉罪类型：" prop="preliminaryJudgment" :rules="[{ required: true, message: '请选择涉罪类型', trigger: 'blur' }]">
                      <el-select
                        filterable
...
export default {
  ...
  computed: {
    searchParams() {
      const obj = {}
      var preliminaryJudgment = this.dialogForm.preliminaryJudgment

      if (!this.dialogForm.preliminaryJudgment) {
        preliminaryJudgment = ''
      }
      if (this.dialogForm.preliminaryJudgment.indexOf('1401') !== -1) {
        // const i = this.dialogForm.preliminaryJudgment.indexOf('1401')
        preliminaryJudgment = '1401-' + this.supplementOne
      }
      if (this.dialogForm.preliminaryJudgment.indexOf('1402') !== -1) {
        // const n = this.dialogForm.preliminaryJudgment.indexOf('1402')
        preliminaryJudgment = '1402-' + this.supplementTwo
      }
      obj.preliminaryJudgment = preliminaryJudgment
      return obj
    }
  },
  mounted() {
  ...
  methods: {
        ...
        if (valid) {
          ...
          centerRoster(this.params).then(res => {
            if (res.code === 200) {
              ...
              this.preliminaryJudgment = this.dialogForm.preliminaryJudgment
    ...
    dataFn() {
      ...
      this.$emit('listFn', {
        ...
        total: total,
        preliminaryJudgment: this.preliminaryJudgment,
