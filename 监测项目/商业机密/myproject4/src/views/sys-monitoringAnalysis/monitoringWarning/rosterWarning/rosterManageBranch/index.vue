<template>
  <div class="rosterManageBranch_main">
    <el-card>
      <div
        slot="header"
      ...
      >
        ...
        <div style="float:right">
          <router-link to="rosterManageBranch/batchImport">
            <el-button type="text">批量导入</el-button>
          </router-link>
          <span class="itemline">|</span>
          <router-link :to="{name:'rosterManageBranch_manualEntry',query: { form: 'fzjg'}}">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <router-link v-if="scope.row.delFlag!==1" :to="{name:'rosterManageBranch_manualEntry',query: { id: scope.row.lcnId}}">
...
// import { ValidQueryInput, adminisValid, delDataValidInput, commonPattern } from '@/utils/formValidate'
import { ValidQueryInput, commonPattern } from '@/utils/formValidate'
import { getTable, delRoster } from '@/api/sys-monitoringAnalysis/roster-warning/roster-branch.js'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // sourceDire: [{ validator: ValidQueryInput, trigger: 'blur' }],
        // sourceBusiness: [{ validator: ValidQueryInput, trigger: 'blur' }],
  ...
  computed: {
    ...
    ...mapGetters(['permissions_routers', 'user_riid'])
  },
  created() {
  ...
  methods: {
    ...
    fetchData(params, obj) {
      ...
      //   params.preliminaryJudgment = this.$refs.judgment.searchParams.join()
      // } else {
      //   params.preliminaryJudgment = ''
      // }
      // params.preliminaryJudgment = params.preliminaryJudgment.join(',')
    ...
    // 查看名单
    viewRoster(uid) {
      this.$router.push({ name: 'rosterManageBranch_detail', params: { rosterId: uid }})
    },
    // 点击查询
    searchFn(obj) {
