<template>
  <el-card class="box-card rosterManage-batchImport">
    ...
    <div slot="header" class="clearfix">
      <span>{{this.ids === 'first'?'来源业务':this.ids === 'second'?'来源方向':'主体类型'}}批量导入</span>
...
export default {
  ...
  computed: {
    ...
    uploadURL() {
      ...
      return `/monitor/listwarn/sensitive/upFile/${id2}?token=${this.token}`
    }
  },
  mounted() {
  ...
  methods: {
    ...
    downloadFn() {
      ...
      location.href = `/monitor/listwarn/sensitive/download/${id}?token=` + this.token
    },
    // 查询历史文件
