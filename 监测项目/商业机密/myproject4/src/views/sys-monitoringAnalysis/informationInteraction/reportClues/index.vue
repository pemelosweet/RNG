    ...
    <el-card>
            ...
            <template v-else>
              ...
              >
                ...
                <el-col :span="8">
                  <el-form-item
                    ...
                    :rules="[{ required: false, trigger: 'change'}]"
                  >
                    <el-select style="width:100%" v-model="form.branchBankName" placeholder="请选择机构" clearable>
                      <el-option
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="8">
                  ...
                  <el-select v-model="form.shareState" clearable placeholder="是否共享给其他分支行">
      ...
      <div class="taleBlock">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              {{scope.row.shareState=='1'?'是':scope.row.shareState=='0'?'否':''}}
            </template>
          </el-table-column>
          <!-- <el-table-column
            prop="shareDt"
...
import { getClues, deleteClue, setState, checkReport, getReportFlag } from '@/api/sys-monitoringAnalysis/report-clues/search.js'
// getEdit
...
export default {
  ...
  computed: {
    ...mapGetters(['roles', 'user_riid', 'user_name']),
    paramsObj() {
    ...
    isTime(val) {
      if (!val) {
        this.timer = setInterval(() => {
          getReportFlag().then(res => {
            if (res.code === 200) {
              this.listType = res.data
              this.isTime = this.listType
            }
          })
        }, 2000)
      } else {
        location.href = '/monitor/reportClue/downloads'
  ...
  methods: {
    ...
    validateNumber(rule, value, callback) {
      if (value !== null && value !== '' && value !== undefined) {
        if (commonPattern.specialCharInQuery.test(value)) {
          callback(new Error('内容不能填写特殊字符'))
        } else if (value.length !== 0 && (value.length <= 5 || value.length >= 129)) {
    ...
    cleanUp() {
      ...
      if (this.roles === 'branch') {
        this.form.branchBankName = this.user_riid
    ...
    exportDataBatch() {
      ...
      if (len > 0) {
        checkReport({ ids: this.select_orderId.join(), token: this.token }).then(res => {
          if (res.code === 200) {
            this.isTime = false
            this.$message({
              message: '正在下载中，请耐心等候！',
