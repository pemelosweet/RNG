  ...
  methods: {
    ...
    handleUpdate(scope) { // 修改
      ...
      if (this.isReport(scope)) {
        ...
        // this.dialogRules.queryNo = [{ required: true, message: '内容不能为空', trigger: 'blur' }, { validator: onlyNumberValidate, trigger: 'blur' }, { validator: isValidInput, trigger: 'blur' }]
      } else {
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            res.data.list.forEach(item => {
              if (item.qttQttcCh.indexOf('大额') === -1) {
                this.list.push(item)
              }
            })
