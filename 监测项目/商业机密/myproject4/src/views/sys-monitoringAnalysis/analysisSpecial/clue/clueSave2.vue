...
import {
  ...
  approvalStaus,
  delAttach
} from '@/api/sys-monitoringAnalysis/analysisSpecial/index2.js'
...
export default {
  ...
  computed: {
    updataUrl1() {
      return `/monitor/cross-special-apply/upAttach?applyId=${this.urldata}&token=${this.token}`
    },
    updataUrl2() {
      return `/monitor/cross-special-apply/upFile?token=${this.token}`
    },
    updataUrl3() {
      return `/monitor/cross-special-apply/upAttachResult?applyId=${this.urldata}&token=${this.token}`
    },
    ...mapGetters([
      'businessFlag',
      'workFlow2business',
  ...
  methods: {
    downAccount() {
      window.location.href =
        '/monitor/cross-special-apply/output?token=' + this.token + '&applyId=' + this.form.applyId
    },
    goback() {
    ...
    downloadResult(file) {
      ...
      // window.location.href = '/monitor/cross-special-apply/downloadAttach?token=' + this.token + '&noteId=58b1f35c70dc4f16926cd3a1d116c035'
      // window.location.href = '/monitor/cross-special-apply/downloadAttach?token=' + this.token + '&fileId=' + file.id
    },
    ...
    callWorkFlow() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$store.dispatch('workFlow', { configCode: 'crossbrApply' })
      ...
      approvalStaus(this.form, this.workFlow2business)
        .then(res => {
          if (res.code === 200) {
            ...
            this.loading = false
            this.$router.push({ name: 'businessManage_query' })
          } else {
            this.loading = false
    ...
    // 下载模板
    // download() {
    //   window.location.href = 'http://192.168.50.77:8084/cross-special-apply/download'
    // },
    // 下载模板
    download() {
      window.location.href = '/monitor/cross-special-apply/download?token=' + this.token
    },
    // 选择文件
    ...
    getRef() {
      ...
      this.url = 'monitor/cross-special-apply/' + this.ref.applyId
    },
    // 初始化页面回显
