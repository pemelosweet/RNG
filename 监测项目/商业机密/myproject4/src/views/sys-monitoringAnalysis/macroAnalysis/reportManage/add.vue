      ...
      <el-form :model="form" label-width="140px" :rules="rules" ref="form">
        ...
        <el-row>
          <el-col :span="20">
            ...
            <el-col :span="20" v-if="form.upLoadData.annexName !== ''">
        ...
        <div style="text-align:center;">
          ...
          <el-button type="primary" plain @click="theStaginga" style="width:100px;margin-left:20px;" :loading="isTrue">暂 存</el-button>
          <el-button type="primary" plain @click="callWorkFlow" style="width:100px;margin-left:20px;" :loading="isTrueOne">提交审核</el-button>
        </div>
      </el-form>
...
import { onSubmit, theStaging } from '@/api/sys-monitoringAnalysis/reportManage/add.js'
export default {
  data() {
    return {
      isTrue: false,
  ...
  methods: {
    ...
    theStaginga() {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          theStaging(this.paramsObjTwo).then(res => {
            this.isTrue = true
            if (res.code === 200) {
              this.isTrue = false
              this.$message({
    ...
    nextStep() {
      this.isTrueOne = true
      onSubmit(this.paramsObj).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '提交成功',
            type: 'success',
