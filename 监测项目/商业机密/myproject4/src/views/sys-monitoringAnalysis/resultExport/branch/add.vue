<template>
  <div class="conjointAnalysisAdd">
    <el-card class="box-card">
      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-form-item label="初步判断：" prop="jub" >
            <el-select style="width:100%" v-model="form.jub" clearable placeholder="请选择" multiple >
        ...
        <el-col :span="24">
          ...
          >
            <el-input
              ...
              :disabled="isUpdate"
              type="textarea"
              v-model="form.transferRecord"
              placeholder="请输入关联移送记录，最多输入200字符"
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col :span="24">
          ...
          >
            <el-input
              ...
              placeholder="请输入监测分析触发点，最多输入200字符"
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col :span="24">
          ...
          >
            <el-input
              ...
              :disabled="isUpdate"
              type="textarea"
              v-model="form.analyseSummary"
              placeholder="请输入分析概述，最多输入200字符"
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
          ...
          >
            ...
            >
              ...
              :disabled="isUpdate"
                ...
                plain
              >点击上传</el-button>
              <div slot="tip" class="el-upload__tip">说明：每个附件上传最大限制50M</div>
            </el-upload>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <div style="text-align: center">
          ...
          >保存</el-button>
          <el-button
          v-if="publicName==='编辑分析结果' && !isUpdate"
          :disabled="disForm"
            type="primary"
            @click="update('form')"
            :loading="loading"
...
import { getSheZui, delAttach, newSave, getInfoById, updateSave, newSubmit, updateSubmit } from '@/api/sys-monitoringAnalysis/resultExportJian/index'
...
export default {
  data() {
    return {
      ...
      form: {
        ...
        receiveDept: '', // 移送方向
        jub: '', // 涉罪类型
        transferRecord: '', // 关联移送记录
        triggerPoint: '', // 监测分析触发点
        analyseSummary: '', // 分析概述
        propose: '', // 建议
        approvalDate: '', // 签批时间
        tradeTime: '', // 交易时间段
      ...
      token: getToken(),
      rules: {
        orCreName: [{ required: true, validator: common_Validate, trigger: 'blur' }],
        receiveDept: [{ required: true, validator: common_Validate, trigger: 'blur' }],
  ...
  mounted() {
    this.form.orId = this.getUUID()

    if (this.$route.query.status === '0') {
      // 判断这条数据的状态是不是已分发
      this.isUpdate = false
      this.disForm = false
      this.publicName = '编辑分析结果'
      this.getInfo(this.$route.query.orId)
    }
    if (this.$route.query.status === '1' || this.$route.query.status === '2' || this.$route.query.status === '3') {
      this.disForm = true
      this.publicName = '查看分析结果'
      this.getInfo(this.$route.query.orId)
    }
    this.getSheZuiLX()
    this.isCenter1()
  ...
  computed: {
    ...
    updataUrl1() {
      return `/monitor/outResult/upAttach?isMain=1&orId=${this.form.orId}&token=${this.token}`
    },
    updataUrl2() {
      return `/monitor/outResult/upAttach?isMain=0&orId=${this.form.orId}&token=${this.token}`
    }
  },
  methods: {
    // 回显
    getInfo(id) {
      getInfoById(id).then(res => {
        console.log(res)
        this.form = res.data
        this.form.jub = this.form.jub.split(',')
        this.form.tradeTime = []
    ...
    //   console.log(this.$refs['Newupload'].uploadFiles)
    // },
    // 编辑
    update(formName) {
      this.$refs[formName].validate(valid => {
    ...
    // 下载模板
    download() {
      window.location.href = '/monitor/outResult/downloadXlsx?token=' + this.token
    },
    // 下载附件
    downFile(file) {
    ...
    upSuccess(res, file) {
      if (res.code === 200) {
        ...
        // const arr = []
        // console.log(this.$refs['Newupload'].uploadFiles)
        // this.$refs['Newupload'].uploadFiles.forEach(item => {
        //   arr.push(item.response.data.attachId)
        // })
        // this.form.fileExtraId = arr.join(',')
        this.$message({
          message: '上传成功！',
          type: 'success'
        })
