      ...
      <div class="content">
        ...
        <div v-show="ifProject">
          <el-table :data="ProjectTableData"
             style="width: 100%"
             v-loading="loadingEstableSubmit"
        ...
        <div v-show="!ifProject">
          <el-table :data="tableData" style="width: 100%"
           ...
           v-loading="loadingEstableSubmitTwo"
           element-loading-text="拼命加载中"
           element-loading-spinner="el-icon-loading"
           element-loading-background="rgba(0, 0, 0, 0.1)">
            ...
            <el-table-column prop="modifyTimestamp" v-if="tableDataArr.length===1" :key="1" label="修改时间" min-width="120" sortable="custom">
              <template slot-scope="scope">
                  {{scope.row.modifyTimestamp | formatDate}}
              </template>
            </el-table-column>
            <el-table-column prop="modifyTimestamp" v-else label="修改时间" :key="2" min-width="120" sortable>
              <template slot-scope="scope">
                  {{scope.row.modifyTimestamp | formatDate}}
              </template>
            </el-table-column>
            <el-table-column prop="description" label="描述" min-width="100"></el-table-column>
            <el-table-column label="审核状态" lable-width="50">
                <template slot-scope="scope">
            ...
            <el-table-column label="操作" lable-width="50">
                <template slot-scope="scope">
                    ...
                    <!-- </router-link> -->
                    <!-- <router-link :to='{name: "modelPlatformCR_lookDataCollection",query:{aId:encodeURI(scope.row.path)}}'> -->
                      <el-button type="text" size="medium" v-if="scope.row.status==='1'" @click="twoLookIndex(scope)">查看</el-button>
                    <!-- </router-link> -->
                </template>
            </el-table-column>
          </el-table>
...
import {
  ...
  onClickSubmitPath
  // onClickSubmitPro
} from '@/api/sys-monitoringAnalysis/modelInfoDataCR/modelSubmitAudit'
...
export default {
  ...
  mounted() {
    ...
    const ArrOBj = JSON.parse(sessionStorage.getItem('mySophon'))
    if (ArrOBj.twoCome) {
      this.ifProject = false
      this.projectId = ArrOBj.id
      this.projectName = ArrOBj.projectName
      const babObj = {
        recursive: true,
        // pid: '96cd91ff-0956-45c0-a85d-2743397b8665',
        pid: ArrOBj.id,
      ...
      sessionStorage.removeItem('mySophon')
      fuzzyQuery(babObj).then(res => {
        if (res.code === 200) {
  ...
  methods: {
    ...
    stopJumpIndex(scope) {
      onClickSubmitPath(encodeURI(scope.row.path)).then(res => {
        if (res.code === 200) {
          console.log(this.projectName, res.data)
          this.$router.push({
            name: 'modelPlatformCR_submitExperiments',
            query: {
              nameTableInfo: scope.row.name,
              pathAid: scope.row.path,
              name: this.projectName,
              modelKey: this.projectId + '_' + this.UUID,
              id: this.projectId
            }
          })
          const obj = {}
          obj.id = this.projectId
          obj.comeOnBloar = true
    ...
    twoLookIndex(scope) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          // name: this.projectName,
          // id: this.projectId
          modelKey: this.projectId + '_' + scope.row.createTimestamp,
          status: scope.row.auditType === '1' ? '审核通过' : '待审核',
          aId: encodeURI(scope.row.path)
        }
      })
    }
  }
