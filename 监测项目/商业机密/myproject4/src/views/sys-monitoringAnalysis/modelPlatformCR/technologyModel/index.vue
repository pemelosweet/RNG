  ...
  <div class="technologyModel">
    <el-card>
      ...
      <el-form ref="modelForm" :model="form" :rules="modelFormRules" label-width="115px">
            ...
            <el-row>
                <el-col :span="8">
                <el-form-item label="模型类型：" prop="suspectOffenceType">
    ...
    <div class="titlefix" style="margin:10px">
      ...
      <el-table :data="data"
        ...
        <el-table-column prop="modelAId" label="名称" min-width="250" show-overflow-tooltip>
          <!-- <template slot-scope="scope">
              <el-button type="text">{{scope.row.modelName}}</el-button>
          </template> -->
        </el-table-column>
        <el-table-column prop="modelKey" label="模型标识" min-width="250">
          <template slot-scope="scope">
        ...
        <el-table-column prop="suspectOffenceType" label="模型类型" min-width="180" show-overflow-tooltip></el-table-column>
        <el-table-column prop="warningRang" label="预警范围" min-width="180" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button type="text" class="buttonListName" @click="showMoreRange(scope)"><span class="buttonSpan">{{ scope.row.warningRang }}</span></el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="270" fixed="right">
          <template slot-scope="scope">
              <router-link :to='{name: "modelPlatformCR_modelInfoIndex"}'>
                  <el-button  type="text" size="medium">查看详情</el-button>
                </router-link>
               <!-- <router-link :to='{name: "modelPlatformCR_lookDataCollection",query:{aId:encodeURI(scope.row.modelAId), modelOnlyId:scope.row.modelOnlyId}}'> -->
                  <el-button  type="text" size="medium" @click="lookSubmitInfo(scope)">查看提交信息</el-button>
                <!-- </router-link> -->
      ...
      <el-pagination v-if="this.total" @size-change="handleSizeChange" @current-change="handleCurrentChange" background :current-page="pageInfo.pageNum" :page-size="pageInfo.pageSize" :page-sizes="[10, 20, 30, 40,50]" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </el-card>
        <!--审核-->
    <el-dialog title="审核" :visible.sync="dialogAuditListVisible" width="40%" class="personDialog">
      <el-form ref="auditList" :model="auditList" label-width="100px" style="border:none">
        ...
        <el-form-item label="审核意见：" style="padding:10px" prop="auditOpinion">
          <template>
            <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 10}" :maxlength='80' placeholder="长度不能超过80" v-model="auditList.auditOpinion"></el-input>
          </template>
        </el-form-item>
      </el-form>
...
import {
  ...
  toaudit
  // lookSubtoaudit
} from '@/api/sys-monitoringAnalysis/technologyModelCR/index'
...
export default {
  ...
  mounted() {
    if (sessionStorage.getItem('searchDataTechno')) {
      const searchData = JSON.parse(sessionStorage.getItem('searchDataTechno'))
      if (searchData.pageName === this.$route.name && searchData.ifReviewTechno) {
        this.pageInfo = searchData.pageInfo
        this.form = searchData.searchForm
      }
      sessionStorage.removeItem('searchDataTechno')
    }
  ...
  methods: {
    ...
    getList() {
      techSearch(this.paramsObj).then(res => {
        if (res.code === 200) {
          ...
          const searchDataTechno = {
            pageName: this.$route.name,
            pageInfo: this.pageInfo,
            searchForm: this.form
          }
          sessionStorage.setItem('searchDataTechno', JSON.stringify(searchDataTechno))
        } else {
          this.loadingtechno = false
          this.$message({
            type: 'error',
    ...
    lookSubmitInfo(scope) {
      // lookSubtoaudit(scope.row.modelOnlyId).then(res => {
      //   if (res.code === 200) {
      this.$router.push({
        name: 'modelPlatformCR_lookDataCollection',
        query: {
          aId: encodeURI(scope.row.modelAId),
          status: scope.row.auditType,
          modelOnlyId: scope.row.modelOnlyId
        }
      })
      //   }
      // })
    ...
    sendAssemblyData() {
      const params = {
        ...
        modelOnlyId: this.scope.row.modelOnlyId,
        auditOpinion: this.auditList.auditOpinion,
        modelKey: this.scope.row.modelKey
      }
    ...
    deleteModel(scope) {
      this.$confirm('请确认是否删除当前选中的特征？', '提示', {
        ...
        const obj = {
          id: encodeURI(scope.row.modelAId),
          modelOnlyId: scope.row.modelOnlyId,
          createId: scope.row.createId,
          auditType: encodeURI(scope.row.auditType)
        }
        goDeleteds(obj).then(res => {
          if (res.code === 200) {
            this.$message({
              type: 'success',
              message: '删除成功!',
    ...
    batchDelete() {
      if (this.selOrderId.length > 0) {
        this.$confirm('确定要删除选中的数据？', '提示', {
          ...
          .then(() => {
            const obj = {
              id: encodeURI(this.selOrderId.join(',')),
              modelOnlyId: this.allId.join(','),
              createId: this.createdId.join(','),
              auditType: encodeURI(this.auditTypeArr.join(','))
            }
            this.delRosterFn(obj).then(res => {
              if (res.code === 200) {
                this.$message({
                  message: '删除成功',
                  type: 'success',
      ...
      if (val) {
        val.forEach(val => {
          if (val) {
            ...
            this.allId.push(val.modelOnlyId)
            this.createdId.push(val.createId)
            this.auditTypeArr.push(val.auditType)
          }
        })
      }
    },
