        ...
        <div class="reportBox" style="display:none;width:100%;height:auto;">
          ...
          <el-col :span="24">
            <el-form-item label="举报事由：">
              {{tabJson.report.reporterReasonName}}
            </el-form-item>
          </el-col>
          <!-- <el-col :span="10">
  ...
  methods: {
    ...
    workflowFn() {
      inWorkflow(this.id).then(res => {
        if (res.code === 200) {
          if (res.data.report.reporterReasonName.indexOf('涉嫌其他犯罪的可疑交易行为') !== -1) {
            res.data.report.reporterReasonName = res.data.report.reporterReasonName.replace(/涉嫌其他犯罪的可疑交易行为/, '涉嫌其他犯罪的可疑交易行为' + '-' + res.data.report.reporterRemark)
          }
